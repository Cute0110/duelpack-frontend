...
const SideSliderScreen = ({ }: any) => {
    ...
    return (
        <div ref={containerRef} className='w-full h-full overflow-y-hidden p-4 relative'>
            {tempItem &&
                <div className="absolute z-[1] left-1/2-translate-x-1/2 slider-item rounded-xl w-[218px] aspect-square cursor-pointer group bg-[#22272b] py-4 mt-4">
            ...
            >
                {sliderItems.map((data: any, index: any) =>
                    <div className="slider-item rounded-xl w-full aspect-square cursor-pointer group bg-[#22272b] relative py-4 mt-4" key={index}>
                        <div className="flex items-center justify-end w-full px-4">
                            <img src="/images/users/default.png" className="rounded-full w-6" />
                        </div>
