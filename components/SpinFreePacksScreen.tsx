...
import { Minus, Plus, Check, Zap, RotateCw, LockKeyhole } from "lucide-react";
...
const SpinFreePacksScreen = ({ packsInfo, packItemConnectInfo, packId, onBuyItemAction, isMobile, onSpinFinishAction }: any) => {
  ...
  const [spinRemainTime, setSpinRemainTime] = useState("24:00:00");
  ...
  const onStartSpin = (method: any) => {
    ...
    if (method == true) {
      ...
      onBuyItemAction(addedPackIds, addedItems, authData.id, addedPacksTotalPrice, addedPacks[0].packInfo.type);
    }
    if (isSpinFaster) {
        ...
        <div className="w-full">
          <div className="flex items-center justify-between py-4">
            <div className="flex items-center gap-2 md:gap-4">
              ...
              <button type="button" className="flex h-[40px] items-center gap-2 bg-gray-600 hover:bg-gray-500 text-white rounded-md p-2 font-bold disabled:cursor-not-allowed" onClick={onClickDemoSpin} disabled={isSpin}>
              ...
              addedPacks.map((data: any, index: any) =>
                <div key={index} className="mt-6">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-lg font-bold">{data.packInfo.name}</span>
                  </div>
...
export default SpinFreePacksScreen;
