...
import FreePacksScreen from "@/components/FreePacksScreen";
...
const FreePacks = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const userResponse = await axiosInstance.get('/api/check_session', {
          withCredentials: true,
        });
  
        const userRes = dot(userResponse.data);
  
        if (userRes.status == 1) {
          setAuthData(userRes.userData);
          setIsAuthenticated(userRes.status);
        } else {
