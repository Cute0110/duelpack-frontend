...
const SpinFreePacks = () => {
  ...
  const onBuyItemAction = async (packIds: any, itemIds: any, userId: any, payAmount: any, packType = 1) => {
    try {
      const response = await axiosInstance.post('/api/buy_items', eot({ packIds, itemIds, userId, payAmount, packType }));
      const res = dot(response.data);
      if (res.status == 1) {
      } else {
        openNotification("error", "Error", res.msg, "topRight");
