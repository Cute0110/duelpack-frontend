...
import ProfileWithdrawalsScreen from "@/components/Profile/Withdrawals";
...
const ProfileWithdrawalsPage = () => {
        ...
        const fetchData = async () => {
            try {
                ...
                if (res.status == 1) {
                    ...
                    const depositResult = await axiosInstance.post('/api/user_withdraw_history', eot({ userId: res.userData.id, search: "", start: 0, length: 10, order: "id", dir: "ASC" }));
    ...
    const onGetTableDataAction = async (search: any, start: any, length: any, orderData: any) => {
        try {
            ...
            if (userRes.status == 1) {
                const result = await axiosInstance.post("api/user_withdraw_history", eot({ userId: userRes.userData.id, search, start: (start - 1) * length, length, order: orderData.order, dir: orderData.dir }));
